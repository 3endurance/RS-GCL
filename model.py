...
class LightGCL(nn.Module):
    def __init__(self, n_u, n_i, d, u_mul_s, v_mul_s, ut, vt, train_csr, adj_norm, l, temp, lambda_1, dropout,
                 batch_user, device):
        ...
        self.G_u1_list = [None] * (l + 1)
        self.G_i1_list = [None] * (l + 1)
        ...
        if test == True:  # testing phase
            ...
            for layer in range(1, self.l + 1):
                ...
                self.Z_u_list[layer] = self.act(
                    spmm(sparse_dropout(self.adj_norm, self.dropout), self.E_i_list[layer - 1], self.device))
                ...
                self.G_u1_list[layer] = self.act(self.u_mul_s @ vt_ei)
                vt_ei += vt_ei
                self.G_u_list[layer] = self.act(self.u_mul_s @ vt_ei)

                ut_eu = self.ut @ self.E_u_list[layer - 1]
                self.G_i1_list[layer] = self.act(self.v_mul_s @ ut_eu)
                ut_eu += ut_eu
                self.G_i_list[layer] = self.act(self.v_mul_s @ ut_eu)

                # aggregate
                self.E_u_list[layer] = self.Z_u_list[layer] + self.E_u_list[layer - 1] * 0.7
                self.E_i_list[layer] = self.Z_i_list[layer] + self.E_i_list[layer - 1] * 0.7

            # aggregate across layers\
            # weights = [1/(2**(i+1)+1) for i in range(self.l+1)]
            # self.E_u = sum([w*self.E_u_list[i] for i, w in enumerate(weights)])
            ...
            for l in range(1, self.l + 1):
                ...
                gnn_u1 = nn.functional.normalize(self.Z_u_list[l][uids], p=2, dim=1)
                hyper_u1 = nn.functional.normalize(self.G_u1_list[l][uids], p=2, dim=1)
                hyper_u1 = self.Ws[l - 1](hyper_u1)
                pos_score1 = torch.exp((gnn_u1 * hyper_u1).sum(1) / self.temp)
                neg_score1 = torch.exp(gnn_u1 @ hyper_u1.T / self.temp).sum(1)
                # pos_score = 0.5*pos_score1 + 0.5*pos_score
                #
                # neg_score =0.5* neg_score1 + 0.5*neg_score
                pos_score = 0.5 * pos_score1 + 0.5 * pos_score

                neg_score = 0.5 * neg_score1 + 0.5 * neg_score
                loss_s_u = torch.sum((-torch.log(1e-8 + torch.sigmoid(pos_score)) - torch.log(1e-8 + (1 - torch.sigmoid(neg_score))))*u_mask)
                ...
                hyper_i1 = nn.functional.normalize(self.G_i1_list[l][iids], p=2, dim=1)
                hyper_i1 = self.Ws[l - 1](hyper_i1)
                pos_score1 = torch.exp((gnn_i * hyper_i1).sum(1) / self.temp)
                neg_score1 = torch.exp(gnn_i @ hyper_i1.T / self.temp).sum(1)
                pos_score = 0.5 * pos_score1 + 0.5 * pos_score
                neg_score = 0.5 * neg_score1 + 0.5 * neg_score
                loss_s_i = torch.sum((-torch.log(1e-8 + torch.sigmoid(pos_score)) - torch.log(1e-8 + (1 - torch.sigmoid(neg_score))))*i_mask)
